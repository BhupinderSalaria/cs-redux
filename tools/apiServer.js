...
server.post("/players/", function(req, res, next) {
  ...
  if (error) {
    ...
    req.body.slug = createSlug(req.body.name); // Generate a slug for new players.
    next();
  }
});
...
function validatePlayer(player) {
  if (!player.name) return "Name is required.";
  if (!player.coachId) return "Coach is required.";
  if (!player.game) return "Game is required.";
  return "";
}
